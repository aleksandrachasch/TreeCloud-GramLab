...
//	final String [] fontcoloryahoo = {"#dfdfdf", "#B8C9D6", "#668AA8",
//			"#477B7B", "#E76500", "#E76200", "#E76200", "#E76200", 
//			"#E76200", "#FF3300"};
...
//	final String [] fontcolorberry = {"#dcdcdc", "#C8C8C8", "#B4B4B4", "#a0a0a0", 
//			"#8c8c8c", "#787878", "#464646", "#323232", "#1E1E1E", "#141414", "#000000" };
//	
...
//					 String concor = item.getAbsolutePath();
//				}
//			}else{
...
//		}
    ...
    public ArrayList<String> readConcor(String concorpath) throws IOException{ 
		...
		//FileWriter fw = new FileWriter("C:/preprocessed_concordance.txt");
    ...
    /*
     * TODO: parameter min nb of occurrences does not have any effect (only number of words in a tree is taken into account)
     */
    /**
    ...
    public Map<String, Integer> getWordsMinFreq(Map<String, Integer> freqs, int minnb){
    	...
    	//System.out.println("wordsminfreq: " + result.size());
    	return result;
    }
...
//		  int i = 0;
//		  int j = freqs.size()-1;
//		  while(j>=0 && freqs.get(j)>= minnb){
//			  ArrayList<String> thesewords = new ArrayList<String>(freqsdict.get(freqs.get(j)).keySet());
//			  int k = 0;
//			  while(k<thesewords.size()){
//				  if(!thesewords.get(k).equals("aaaaaaaaa") && !stopwords.contains(thesewords.get(k)) && i<nbwords | thesewords.get(k).equals(locatetarget)){
//					  wordsID.put(thesewords.get(k), i);
//					  i += 1;
//				  }
//				  k += 1;
//			  }
//			  j += -1;
//		  }
		  //System.out.println("WordsID length: " + wordsID.size());
    ...
    /*
     * TODO: add more methods for computing distance 
     */
    ...
    /*
     * TODO: add another method for normalizing the matrix
     */
    ...
    private ArrayList<Map<String, Double>> computeCooccurrenceColors(String theword, ArrayList<String> text, Map<String, Integer> keptWordsId, ArrayList<ArrayList<Double>> distance){
    	...
    	for(String word : keptWords){
    		if(!positions.containsKey(word)){
    			...
    			//double tmp = Math.max(value, themax);
    			//themax = tmp;
    			
    			//if(!theword.equals(word)){
    				//double tmp2 = Math.min(value, themin);
    				//themin = tmp2;
    	...
    	double themax = Collections.max(positions.values());
    	double themin = Collections.min(positions.values());
    	...
    	Map<Double, String> backitems = new TreeMap<Double, String>(tmp);
    	System.out.println("backitems: " + backitems);
    ...
    public String setLabelsColor(ArrayList<TreeNode> nodes, Map<String, Integer> keptWordsId, int [] frequencies, ArrayList<String> text, String color, ArrayList<ArrayList<Double>> distance){
    	...
    	for(TreeNode node : nodes){
    		System.out.println("-----------");
    		System.out.println("word: " + node.name);
    		int averagepos = 0;
    		int averagepos2 = 0;
    		if(chronology>0){
    			if(chronology==1){
    				...
    				System.out.println("averagepos2: " + averagepos2);
    			}
    			if(chronology==2){
    			...
    			System.out.println("value: " + (int) (1+9.99999*(Math.log(frequencies[keptWordsId.get(node.name)])-Math.log(themin))/(Math.log(themax)-Math.log(themin))));
    			...
    			if(color.equals("berry")){
    				...
    				System.out.println((int) (1+9.99999*(Math.log(frequencies[keptWordsId.get(node.name)]-Math.log(themin))/(Math.log(themax)-Math.log(themin)))));
...
//    	ArrayList<String> stopwords = new ArrayList<String>();
//    	
//    	FileReader fw = new FileReader("C:/stopwords/StoplistEnglishFrench.txt");
//    	BufferedReader bf = new BufferedReader(fw);
//    	while(true){
//    		String line = bf.readLine();
//    		if(line == null){
...
//    		}
//    	}
//    	ArrayList<String> conc = ct.readConcor("C:/Users/SONY/Desktop/GSoC/unitex_proposal/bush_snt/concord.html");
//    	ArrayList<String> words = ct.text(conc);
//    	Map<String, Integer> freqs = ct.freqsDict(words);
//    	Map<Integer, Map<String, Integer>> freqsdict = ct.sortByFrequency(freqs);
//    	ArrayList<String> cleaned = ct.removeStopWords(freqs, stopwords);
//    	Object ob [] = ct.wordList(freqsdict, freqs, cleaned, 30);
...
//		   ConcordanceText t = new ConcordanceText();
//		   t.FindFiles("C:/Users/SONY/Documents/Unitex-GramLab/Unitex/English/Corpus");
//		   t.readConcor();
//		   //System.out.println(t.getText());
//		   //System.out.println(t.getFrequencies().size());
//		   
//		   PreprocessConcordance c = new PreprocessConcordance();
//			
//		   c.readStats();
//		   ArrayList<String> imposedwords = c.getWordsNumber(c.removeStopWords(c.readStats(), "English"), 30);
//		   
//		   Object [] tet = t.wordList(t.getSortedByFreqs(), t.getFrequencies(), imposedwords);
//		   Map<String, Integer> ids = (Map<String, Integer>) tet[0];
//		   for(String s : ids.keySet()){
//			   System.out.println(s + ": " + ids.get(s));
//		   }
//		   //System.out.println(c.removeStopWords(c.readStats(), "English").size());
//		   
//		   System.out.println(ids.size());
//		   
//		   System.out.println(t.filterText(t.getText(), ids));
//		   
//		   ArrayList<ArrayList<ArrayList<Integer>>> coocc = t.computeCooccurrenceDisjoint(ids, t.filterText(t.getText(), ids));
//		   
//		   ArrayList<ArrayList<Double>> dm = t.distanceFromCooccurrence(coocc);
//		   ArrayList<ArrayList<Double>> ndm = t.normalizeMatrix(dm);
//		   
//		   System.out.println(ndm);
//		   
//		   NeighborJoining NJ = new NeighborJoining();
//		   
//		   for(String s: imposedwords){
//				TreeNode n = new TreeNode();
//				n.setName(s);
//				n.setAsLeaf();
//				n.setID(imposedwords.indexOf(s));
//				NJ.allnodes.add(n);
//			}
//		   
//		    ArrayList<String> wo = new ArrayList<String>();
//		    String [] wor = (String[]) tet[1];
//		    for(String s : wor){
//		    	wo.add(s);
//		    }
//		    String newick = NJ.computeNJTree(ndm, wo);
//			System.out.println(newick);
//			EqualAngle ea = new EqualAngle();
//			ea.doEqualAngle(NJ.allnodes, 28);
//			
//			TreeSVG.drawTreeCloud(NJ.allnodes);
////			nodes = nj.allnodes;
//	   }
//}
